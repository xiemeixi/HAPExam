...
(function ($)
{
        ...
        _init: function ()
        {
            ...
            if ($(this.element).attr("ligerui"))
            {
                try
                {
                    ...
                    // if (attroptions.indexOf('{') != 0) attroptions = "{" + attroptions + "}";//remove by huazhen.wu@hand-china.com
            ...
            //add by huazhen.wu@hand-china.com start
            if ($(this.element).attr("validate")){
                var validate;
                eval("validate="+$(this.element).attr("validate"));
    ...
    liger.controls.Input.ligerExtend(liger.core.UIComponent, {
        ...
        {
            var wrapper = this.wrapper || this.text,p = this.options;
            if (p.disabled || !wrapper || !wrapper.hasClass("l-text")) return;
            ...
            if (readonly)
            {
                ...
                if(this._setCancelable) this._setCancelable(false);//add by njq.niu@hand-china.com
            } else
            {
                if (inputText) inputText.removeAttr("readonly");
                wrapper.removeClass("l-text-readonly");
                if(this._setCancelable) this._setCancelable(true);//add by njq.niu@hand-china.com
            }
        },
        ...
        //add by huazhen.wu@hand-china.com start
        setTypeCase:function(typecase){
            return this.set('typeCase',typecase);
        },
        //设置必输
    ...
    liger.win = {
        ...
        setFront: function (win)
        {
            ...
            var pre = this.queue[this.queue.length-1];
            if(pre == win){
                pre = this.queue[this.queue.length-2];
            }
            if(pre){
                $(pre.element).css("z-index", "8900");
            }
            $(win.element).css("z-index", "9100");
            //modify by huazhen.wu@hand-china.com end 
        },
        //add by huazhen.wu@hand-china.com start
        queue:[],
    ...
    liger.core.Win.ligerExtend(liger.core.UIComponent, {
        ...
        _init: function()
        {
            liger.core.Win.base._init.call(this);
            liger.win.add(this);
        },
        //add by huazhen.wu@hand-china.com end
        mask: function ()
...
//                if (e.password) inputType = "password";
                ...
                if (e.password) {
                    inputBody.focus(function(){
                        inputBody.attr('type','password');
                    })
                }
                //modify by njq.niu@hand-china.com end
                ...
                if (field.editor)
                {
                    ...
                    // add by chenjingxiong@hand-china.com begin
                    // 不删除的参数
                    var staticOptions = field.editor.staticOptions || field.editor.sp;
                    if(staticOptions){
                        staticOptions = typeof (staticOptions) == 'function' ? staticOptions(editParm) : staticOptions;
                        $.extend(p, staticOptions);
                    }
                    // add by chenjingxiong@hand-china.com end
                ...
                }*///remove by huazhen.wu@hand-china.com
                return lobj;
            },
            getValue: function (editor, editParm)
            {
                ...
                if (editor.getValue)
                {
                    ...
                    //modify by huazhen.wu@hand-china.com start
                    if (edtirType == "int" || edtirType == "digits" || edtirType == "float" || edtirType == "number"){
                        if ($.isEmpty(value))
                        {
                            return '';
                        }
            ...
            { 
                ...
                if (editor.setValue)
                {
                    ...
                    //modify by huazhen.wu@hand-china.com start
                    if(editor.setValue.length>2){
                        editor.setValue(value,undefined,!!editParm.column);
                    }else{
                        editor.setValue(value,!!editParm.column);
                    }
                    //modify by huazhen.wu@hand-china.com end
                } 
            },
...
(function ($)
{
    ...
    $.ligerDefaults.CheckBox = {
        disabled: false,
        readonly : false, //只读
        checkValue:true, // 选择时的值 add by chenjingxiong@hand-china.com 2016-01-27
        uncheckValue:false // 未选择的值 add by chenjingxiong@hand-china.com 2016-01-27
    };
    ...
    $.ligerui.controls.CheckBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            g.input = $(g.element);
            //modify by huazhen.wu@hand-china.com start
            g.link = $('<a class="l-checkbox" tabindex="0"></a>');
        ...
        _getValue: function ()
        {
            ...
            if(this.element.checked){
                return this.options.checkValue;
            } else {
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        initValue: null,
        value : "",//modify by huazhen.wu@hand-china.com
        initText: null,
        valueField: 'id',
        textField: 'text',
        dataParmName : null,
        ...
        autocomplete: true,  //自动完成    modify by njq.niu@hand-china.com
        autocompleteAllowEmpty : true, //是否允许空值搜索
        highLight: true,    //自动完成是否匹配字符高亮显示 // modify by huazhen.wu@hand-china.com
        readonly: false,              //是否只读
        ajaxType: 'post',
        ...
        //keySupport: false,              //按键支持： 上、下、回车 支 //remove by huazhen.wu@hand-china.com
        initIsTriggerEvent: false,      //初始化时是否触发选择事件
        conditionSearchClick: null,      //下拉框表格搜索按钮自定义函数
        focusWhenSetValue : true // 设值时是否聚焦 add by chenjingxiong
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        setCustomParam : function(name, value) {
            ...
            if(name instanceof Object) {
                this.options.customParms = name;
            } else {
                this.options.customParms = new Object();
                this.options.customParms[name] = value;
            }
        },
        _render: function ()
        {
            //modify by njq.niu@hand-china.com
            $.ligerui.controls.ComboBox.base._render.call(this);
            var g = this, p = this.options;
            g.data = p.data;
            g.inputText = null;
            g.select = null;
            ...
            if ($(this.element).is(":hidden"))
            {
                ...
                g.inputText = $('<input type="text" autocomplete="off" readonly="true"/>');//避免默认的autocomplete njq.niu@hand-china.com
            ...
            else if (this.element.tagName.toLowerCase() == "input")
            {
                ...
                g.inputText.attr('autocomplete','off');//避免默认的autocomplete njq.niu@hand-china.com
            ...
            }).click(function ()
            {
                ...
                if (p.triggerToLoad && !g.triggerLoaded)
                {
                    ...
                    //modify by huazhen.wu@hand-cina.com start
                    g.selectBox.table.html('<tr><td>正在查询...</td></tr>');
                    g._toggleSelectBox(g.selectBox.is(":visible"));
                    g._setUrl(p.url);
                    //modify by huazhen.wu@hand-cina.com end
                } else
                {
                    g._toggleSelectBox(g.selectBox.is(":visible"));
                }
            ...
            g.inputText.blur(function ()
            {
                ...
                //modify by njq.niu@hand-china.com start
                if(p.autocomplete == false) return;
                var text = g.inputText.val(), v;
                $.each(g.data_bak||g.data,function(i,d){
                    if(text == d[g.options.textField]){
                        v = d[g.options.valueField];
                        return false;
                    }
                });
                if(v) {
                    g._changeValue(v,text, false);
                }else{
                    g.clear();
                }
            ...
            .keydown(function(e){
                ...
                if(!g.selectBox.is(":visible")){
                    ...
                    if(code == 38 || code == 40){
                        var cur = $('table.l-table-nocheckbox td.l-over',g.selectBox),item;
                        if(cur.length){
                            cur.removeClass('l-over');
                            item = cur.parent()[code == 38?'prev':'next']('tr').children('td');
                        }
                        if(!item||!item.length){
                            item = $('table.l-table-nocheckbox td'+(code == 38?':last':':first'),g.selectBox);
                        }
                        item.addClass('l-over');
                    }else if(code == 13){
                        g._selectItem.call($('table.l-table-nocheckbox td.l-over',g.selectBox),{data:g});
                    }else if(code == 9){
            ...
            $(document).bind("click.combobox", function (e)
            {
                ...
                //这里去掉selectBox的判断引起datepicker无法正常工作.njq.niu@hand-china.com
//                if ($((e.target || e.srcElement)).closest(".l-box-select, .l-text-combobox").length == 0)
//                {
//                    g._toggleSelectBox(true);
//                }
            });
        ...
        _setCancelable: function (value)
        {
            ...
            if (!value && g.unselect)
            {
                g.unselect.remove();
                g.unselect = null;
                g.wrapper.unbind('mouseenter mouseleave');//add by huazhen.wu@hand-china.com
            }
        ...
        _setDisabled: function (value)
        {
            ...
            if (value)
            {
                ...
                this._setCancelable(false);
            } else
            {
                //modify by njq.niu@hand-china.com
                if(p.autocomplete && p.readonly !== true) this.inputText.removeAttr("readonly");
                this.wrapper.removeClass('l-text-disabled');
                if(this.options.cancelable){
                    this._setCancelable(true);
                }
            }
            p.disabled = value;
        },
        //modify by huazhen.wu@hand-china.com end
        //modify njq.niu by huazhen.wu@hand-china.com start
        _setReadonly: function (readonly)
        {
            if(this.options.autocomplete == false) {
                ...
                $.ligerui.controls.ComboBox.base._setReadonly.call(this,readonly);
            }
        },
        ...
        //modify by huazhen.wu@hand-china.com start
        _setValue: function (value, text,inGrid)
        {
            var g = this, p = this.options;
            ...
            {
                g._changeValue(value, text, isTriggerEvent,inGrid);
        ...
        selectValue: function (value,inGrid)
        {
            this._setValue(value,undefined,inGrid);
        },
        //modify by huazhen.wu@hand-china.com end
                    ...
                    if (g.trigger('beforeSetData', [data]) == false)
{
            ...
            g.ajaxId = $.ajax(ajaxOp);//modify by huazhen.wu@hand-china.com
        },
        ...
        setData: function (data)
        {
            ...
            if(p.host_grid_column && p.host_grid_row){
                if(!p.host_grid_row.__combobox_datas){
                    p.host_grid_row.__combobox_datas = {};
                }
                p.host_grid_row.__combobox_datas[p.host_grid_column.columnname] = data;
            }
            //add by huazhen.wu@hand-china.com end
            if (g.select) return;
        ...
        _changeValue: function (newValue, newText,isSelectEvent,inGrid)//modify by huazhen.wu@hand-china.com
        {
            var g = this, p = this.options,
                oldValue = g.valueField.val();//add by huazhen.wu@hand-china.com
            ...
            // edit by chenjingxiong
            if (isSelectEvent && newText && p.focusWhenSetValue)
            {
                g.inputText.focus();
            }
                ...
                success: function (result)
{
        ...
        _addClickEven: function ()
        {
            var g = this, p = this.options;
            //选项点击
            $(".l-table-nocheckbox td", g.selectBox).click(g,g._selectItem); 
        },
        _selectItem: function (e)
        {
            var g = e.data,p = g.options;
            var jcell = $(this);
        ...
        _setAutocomplete: function (value)
        {
            ...
            g.inputText.keydown(function (event)
            {
                ...
                //modify by huazhen.wu@hand-china.com end
                // add by chenjingxiong start
                if (p.readonly || p.disabled) {
                    return;
                }
                // add by chenjingxiong end
                
                //add by njq.niu@hand-china.com start
                function filter(key){
                    g._resetData()
                    if(key) {
                        var reg = new RegExp(key.replace(/[+?*.^$\[\](){}\\|]/g,function(v){
                            return '\\'+v;
                        }),'i'),list = [];
                        $.each(g.data,function(i,d){
                            if(reg.test(d[g.options.textField])!==false){
                                list.push(d); 
                            }
                        });
                        g.data_bak = g.data;
...
*///remove by huazhen.wu@hand-cina.com
...
(function ($)
{
    ...
    $.ligerDefaults.DateEditor = {
        ...
        showTime: false,
        onChangeDate: false,
        onEnterDown: null,//add by huazhen.wu@hand-china.com
        absolute: true,  //选择框是否在附加到body,并绝对定位
        cancelable: true,      //可取消选择
        readonly: false              //是否只读
    };
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            //modify by njq.niu@hand-china.com
            $.ligerui.controls.DateEditor.base._render.call(this);
            var g = this, p = this.options;
            ...
            *************/
            ...
            }).click(selectDate);//modify by huazhen.wu@hand-china.com
            ...
            //上个月
            g.buttons.btnPrevMonth.click(prevMonth);//modify by huazhen.wu@hand-china.com
            //下个月
            g.buttons.btnNextMonth.click(nextMonth);//modify by huazhen.wu@hand-china.com
            //上一年
            ...
            .keydown(function(e)
            {
                ...
                if(g.dateeditor.is(":visible")){
                    switch(e.keyCode){
                        case 37:goLeft(e);break;
                        case 38:goUp(e);break;
                        case 39:goRight(e);break;
                        case 40:goDown(e);break;
                        case 13:selectDate.call($("td.l-box-dateeditor-over", g.body.tbody));break;
                        case 9:g.toggleDateEditor(true);
                        default:{
                            $("td.l-box-dateeditor-over", g.body.tbody).removeClass('l-box-dateeditor-over');
                        }
                    }
                }else{
                    if(code == 40){
                    ...
                    {
                        g.trigger('enterdown');
                        if(g.options && g.options.host_form)
                        {
                            g.options.host_form.trigger('enterdown');
                        }
                    } 
                }
            });
            ...
            function goUp(e){
                var td = $("td.l-box-dateeditor-over", g.body.tbody),tr,cell;
                if(td.length){
                    td.removeClass('l-box-dateeditor-over');
                    cell = td.parent().children().index(td);
                    tr = td.parent().prev();
                    if(tr.length){
                        td = tr.children('td:nth('+cell+')');
                    }else{
                        prevMonth();
                        td = g.body.tbody.find('tr:last td:nth('+cell+')');
                    }
                    td.addClass('l-box-dateeditor-over');
                    e.preventDefault();
                }
            }
            function goDown(e){
                ...
                if(td.length){
                    ...
                    if(tr.length){
                        ...
                        nextMonth();
                        td = g.body.tbody.find('tr:first td:nth('+cell+')');
                    }
                }else{
                    td = $("td:first", g.body.tbody);
                }
                td.addClass('l-box-dateeditor-over');
                e.preventDefault();
            }
            function goLeft(e){
                var td = $("td.l-box-dateeditor-over", g.body.tbody),row;
                if(td.length){
                    td.removeClass('l-box-dateeditor-over');
                    row = g.body.tbody.children().index(td.parent());
                    td = td.prev();
                    if(!td.length){
                        prevMonth();
                        td = g.body.tbody.find('tr:nth('+row+') td:last');
                    }
                    td.addClass('l-box-dateeditor-over');
                    e.preventDefault();
                }
            ...
            function goRight(e){
                ...
                if(td.length){
                    ...
                    if(!td.length){
                        nextMonth();
                        td = g.body.tbody.find('tr:nth('+row+') td:first');
                    }
                    td.addClass('l-box-dateeditor-over');
                    e.preventDefault();
                }
        ...
        _setCancelable: function (value)
        {
            ...
            if (!value && g.unselect)
            {
                g.unselect.remove();
                g.unselect = null;
                g.text.unbind('mouseenter mouseleave');//add by huazhen.wu@hand-china.com
            }
            if (!value && !g.unselect) return;
            g.unselect = $('<div class="l-trigger l-trigger-cancel"><div class="l-trigger-icon"></div></div>').hide();
            g.text.hover(function ()
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        closeWhenEsc: null, //按esc时是否关闭dialog //add by huazhen.wu@hand-china.com
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            g.dialog.mousedown(function ()//modify by huazhen.wu@hand-china.com
            {
                l.win.setFront(g);
            });
            //设置事件
        ...
        doClose: function ()
        {
            ...
            l.win.remove(this);//add by huazhen.wu@hand-china.com
            $.ligerui.remove(this);
            g.unmask();
            g._removeDialog();
        ...
        {
            var g = this;
            l.win.remove(g);//add by huazhen.wu@hand-china.com
            l.win.removeTask(g);
            g.dialog.hide();
            g.unmask();
        },
        show: function ()
        {
            ...
            l.win.add(g);//add by huazhen.wu@hand-china.com
            //前端显示 
...
(function ($)
{
    ...
    $.ligerDefaults.Form.editors.textarea =
    {
        ...
        //add by huazhen.wu@hand-china.com start
        setEnabled:function(editor,isEnabled){
            editor.attr("readonly", !isEnabled)[isEnabled?'removeClass':'addClass']('l-text-disabled');
        }
        //add by huazhen.wu@hand-china.com end
    };*/
    ...
    $.ligerDefaults.Form.editorBulider = function (jinput)
    {
        ...
        {
            control = jinput.ligerTextArea(options);//modify by huazhen.wu@hand-china.com
        }
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            if (p.buttons)
            {
                ...
                $(p.buttons).each(function ()
                {
                    ...
                    this.id = $("div:first", jbutton).ligerButton(this).id;//modify by huazhen.wu@hand-china.com
                });
            }
        ...
        {
            //modify by njq.niu@hand-china.com
            var jele = $("[ligeruiid=" + name + "]"); 
            if (!jele.length || !jele.rules) return;
            //modify by njq.niu@hand-china.com
            var oldRule = jele.rules(); 
            if (oldRule) //旧的验证移除验证结果
            {
                ...
                {
                    //modify by huazhen.wu@hand-china.com start
                    jele.parents("div.l-text").removeClass("l-text-required");
            ...
            if (validate.required)
            {
                //modify by huazhen.wu@hand-china.com start
                //验证包括必填规则，添加*
                jele.parents("div.l-text").addClass("l-text-required"); 
...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.grid.attr('tabindex',0);//add by huazhen.wu@hand-china.com
            var gridhtmlarr = [];
            ...
            //选择分页大小 modify by njq.niu@hand-china.com
            if(p.pageSizeBtn !== false){
            ...
            //刷新按钮 modify by njq.niu@hand-china.com
            if(p.refreshBtn !== false){
                gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        loadData: function (loadDataParm,sourceType)
        {
            var g = this, p = this.options;
            g.loading = true;
            //bug 重新查询后清除删除数据 njq.niu@hand-china.com
            g.deletedRows = [];
            g.trigger('loadData');
            var clause = null;
            var loadServer = true;
        ...
        */
        ...
        endEdit: function (rowParm)
        {
            ...
            {//modify by huazhen.wu@hand-china.com start
                var o = g.editor;
                o.input.element.blur();
                g.trigger('sysEndEdit', [o.editParm]);
                g.trigger('endEdit', [o.editParm]);
                if (o.editor.destroy) o.editor.destroy(o.input, o.editParm);
                o.container.remove();
                g.reRender({ rowdata: o.editParm.record, column: o.editParm.column });
                g.trigger('afterEdit', [o.editParm]);
...
//            g._buildPager();
        ...
        _initColumns: function ()
        {
            ...
            $(g.columns).each(function (i, column)
            {
                ...
                column.isSort = column.isSort == true ? true : false; //modify by huazhen.wu@hand-china.com
        ...
        _createHeaderCell: function (column)
        {
            ...
            {
                !p.isSingleCheck && jcell.addClass("l-grid-hd-cell-checkbox");
        ...
        _clearGrid: function ()
        {
            ...
            //编辑器计算器
            g.editorcounter = 0;
            g.gridview1.find('tr.l-grid-hd-row').removeClass('l-checked');//add by huazhen.wu@hand-china.com
        },
        _fixRows : function()
        {
            var g = this, p = this.options;
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i=0;i<data.length;i++)
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.ischeckbox)
                    {
                        //begin:增加是否可选择的控制 njq.niu@hand-china.com
                        var showCheckBox = true;
                        if(p.canSelect){
                             showCheckBox = p.canSelect(item);
                        }                       
                        gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-'+(p.isSingleCheck?'radio':'checkbox')+'" style="width:' + this.width + 'px"><div class="l-grid-row-cell-inner"');
                        if (p.fixedCellHeight)
                            gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
                        else
                            gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
                        gridhtmlarr.push('>');
                        if(showCheckBox) gridhtmlarr.push('<span class="l-grid-row-cell-btn-checkbox"></span>');
                        gridhtmlarr.push('</div></td>');
                        return;
                        //end:增加是否可选择的控制 njq.niu@hand-china.com
                    }
                        //如果是明细列(系统列)
                    ...
                    //add by njq.niu@hand-china.com start
                    var editParm = { record: item, value: item[column.name], column: column};
                    var disableEdit = g.trigger('beforeEdit', [editParm]) == false;
                    if(!this.editor||g.options.enabledEdit == false || disableEdit) {
                        gridhtmlarr.push('l-grid-row-cell-noeditor ');
                    } else {
                        gridhtmlarr.push('l-grid-row-cell-editor-' + this.editor.type + ' ');
                    }
                    //add by huazhen.wu@hand-china.com start
                    if(this.validate && this.validate.required){
                        gridhtmlarr.push('l-text-required '); 
                    }
                    // add by huazhen.wu@hand-china.com end
        ...
        {
            var g = this, p = this.options,gridhtmlarr=[];
            ...
            if (column.isrownumber){
                ...
                if (p.fixedCellHeight)
                    ...
                    gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
                gridhtmlarr.push('>' + (parseInt(rowdata['__index']) + 1) + '</div>');
                return gridhtmlarr.join(''); 
            }
            //modify by huazhen.wu@hand-china.com end
            ...
            //add by huazhen.wu@hand-china.com start
            if(column.validate){
                htmlarr.push(" validate='",liger.toJSON(column.validate),"'");  
            }
            // add by huazhen.wu@hand-china.com end
            //htmlarr.push('<div');
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            //如果有fieldsCode则显示meaning linxixin_
            if(column.fieldsCode) {
                content = column.fieldsCode[content];
            }
        ...
        //add by huazhen.wu@hand-china.com start
        _focusCell: function(obj){
            var ub = this.gridbody,
                h = ub.height(),
                w = ub.width(),
                oo = obj.offset(),
                oot = oo.top,
                ...
                oh = obj.outerHeight(),
                ow = obj.outerWidth(),
                sh = ub[0].scrollWidth > w ? 17 : 0,
                sw = ub[0].scrollHeight > h ? 17 : 0;
            if(oot - uot < 0){
                ub.scrollTop(oot + ust - uot);
            }else if(oot - uot + oh > h - sh){
                ub.scrollTop(oot + ust - uot + oh - h + sh);
            }
            if(ool - uol< 0 ){
                ub.scrollLeft(ool + usl - uol);
            }else if(ool - uol + ow > w - sw){
                ub.scrollLeft(ool + usl - uol + ow - w + sw);
            }
        },
        //add by huazhen.wu@hand-china.com end
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                var editor = p.editors[column.editor.type],
//                    jcell = $(rowcell), offset = jcell.offset(),
                    width = jcell.width(), height = jcell.height(),
                    container = $("<div class='l-grid-editor'></div>").appendTo(g.form?$(g.form):g.grid),//modify by huazhen.wu@hand-china.com

                    //left = 0,
                    //top = 0,
                    pc = jcell.offset(),//position(),
                    pe = $(g.element).offset(),
              ...
              //modify by huazhen.wu#hand-china.com start
                ...
                container
                    ...
                    .show();
                if (column.textField) editParm.text = g._getValueByName(rowdata, column.textField);
                var editorInput = g._createEditor(editor, container, editParm, width, height - 2),
                    inputBody = editorInput.element;
                if(inputBody){
                    inputBody.name = columnname;
                    inputBody.row = rowdata;
                    if(column.validate){
                        inputBody.gridRowCell = obj;
                        $(inputBody).attr('validate', liger.toJSON(column.validate));
                    }
                    $(inputBody).keydown(function(e){
                        if(e.keyCode == 9){
                            e.preventDefault();
                            g.endEdit();
                            g._showEditorBy(e.shiftKey?37:39,column,rowdata);
                        }
                    });
                }
                //modify by huazhen.wu#hand-china.com end
        ...
        //add by huazhen.wu@hand-china.com start
        _showEditorBy: function(dir,column,row){
            var g = this,columns = g.columns;
            if(dir == 39||dir == 37){
                var step = dir == 39?1:-1
                column = columns[column.columnindex+step];
                if(!column){
                    row = g.rows[row['__index']+step];
                    column = columns[dir == 39?0:columns.length-1];
                }
                if(row && g._applyEditor(g.getCellObj(row,column))==false){
                    g._showEditorBy(dir,column,row);
                }
            }
        },
        //add by huazhen.wu@hand-china.com end
        ...
        _getSrcElementByEvent: function (e)
        {
            ...
            if (r.gridbody)
            {
                ...
                r.checkboxcell = $(r.cell).hasClass("l-grid-row-cell-"+(g.options.isSingleCheck?"radio":"checkbox")) ? r.cell : null;
...
//            g.grid.bind("mousedown.grid", function (e)
            $(document).bind("mousedown.grid", function (e)
        //modify by huazhen.wu@hand-china.com end
            {
            ...
            g.grid.bind("keydown.grid", function (e)
            {
                ...
                if(code == 13){
                    g.trigger('enterdown');
                }else if(p.selectable && p.selectByKey &&(code == 38 || code == 40)){
                    e.preventDefault();
                    var len = g.selected.length,
                        selectedIndex = len?g.rows.indexOf(g.selected[len-1])+(code == 38?-1:1):0;
                        rowdata = g.rows[selectedIndex];
                    if(rowdata)
                        g.select(rowdata,e.shiftKey);
                }
            });
            //add by huazhen.wu@hand-china.com end
            ...
            //remove by njq.niu@hand-china.com
...
//                if (g.editor.editing && !$.ligerui.win.masking) g.endEdit();
            ...
            if (src.checkboxall) //复选框全选
            {
                ...
                g.selected = [];
                //begin:增加是否可选择的控制 njq.niu@hand-china.com
                var canSelect = this.options.canSelect;
                for (var rowid in g.records)
                {
                    var ck = true;
                    if(canSelect) ck = canSelect(g.records[rowid]);
                    if(ck){
                        if (uncheck)
            ...
            else if (src.hcelltext) //排序
            {
                ...
                //modify by njq.niu@hand-china.com
                if (src.column.isSort == false || g.rows.length == 0) return;
            ...
            //modify by huazhen.wu@hand-china.com start
                ...
                var row = $(src.row),
                    rowdata = g.getRow(src.data),
                    rowid = rowdata['__id'],
                    rowobj = g.getRowObj(rowid),
                    rowobj1 = g.getRowObj(rowid,true); 
                $((rowobj?[rowobj]:[]).concat(rowobj1||[])).addClass('l-current-row').siblings('.l-current-row').removeClass('l-current-row');
                if (g.enabledCheckbox()) //复选框选择行
                {
                    //复选框
                    ...
                    {
                        setTimeout(function(){g._applyEditor(src.cell)},100);//modify by huazhen.wu@hand-china.com
                    }
                }
                else if (p.selectable != false)
                {
                    ...
                    {
                        g.select(src.data,e.ctrlKey);//modify by huazhen.wu@hand-china.com
                    }
                }
            }
            //modify by huazhen.wu@hand-china.com end
        ...
        select: function (rowParm,ctrlKey)//modify by huazhen.wu@hand-china.com
        {
            var g = this, p = this.options;
            var rowdata = g.getRow(rowParm);
            var rowid = rowdata['__id'];
            ...
            if ((!g.enabledCheckbox() && !ctrlKey) || p.isSingleCheck) //单选//modify by huazhen.wu@hand-china.com
            {
            ...
            if(!this.isSelected(rowParm))//add by njq.niu@hand-china.com
...
(function ($)
{
    ...
    $.ligerDefaults.PopupEdit = {
        ...
        onChangeValue:null,//njq.niu@hand-china.com
        valueFieldCssClass: null,
        //add by huazhen.wu@hand-china.com start
        autocomplete:false, 
        popup:true
    ...
    $.ligerui.controls.PopupEdit.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            //modify by njq.niu@hand-china.com
            $.ligerui.controls.PopupEdit.base._render.call(this);
            var g = this, p = this.options;
            g.inputText = null;
            //文本框初始化
            ...
            g.link.hover(function ()
            {
                if (p.disabled || !p.popup) return;//modify by huazhen.wu@hand-china.com
                this.className = "l-trigger-hover";
            }, function ()
            {
                if (p.disabled || !p.popup) return;//modify by huazhen.wu@hand-china.com
            ...
            {   //njq.niu@hand-china.com
                if (p.disabled) return;
                g.trigger('changeValue', [g.getText()]);
                g.setValue('');//add by huazhen.wu@hand-china.com
            }).blur(function ()
            {
                if (p.disabled) return;
                g.wrapper.removeClass("l-text-focus");
            ...
            if(p.autocomplete){
                g._initAutoComplete();
                var timeout_id = null,queryed = '';
                function _delayRun(callback,delay){
                    if(timeout_id){
                ...
                g.inputText.keypress(function(e){
                    ...
                    if (p.disabled || p.readonly) return;
                    var code = e.keyCode,input = this;
                    if(code > 40 && code < 65 || code > 90 || 
                            (code < 37 && code !=33 && code !=34 && code != 13 && code !=27 && code != 9 && code!=18 && code!=17 && code!=16) ||
                            (queryed != input.value && code == 16)){
                        _delayRun(function(){
                            _delayRun(function(){g._autoCompleteQuery(queryed = input.value)},600);
                        },1)
                    }
                    if(g.autoShown && code == 13){
                        g._commitSelected();
                        e.preventDefault();
                    }
                }).keydown(function(e){
                    if (p.disabled || p.readonly) return;
                    var code = e.keyCode,input = this;
                    if(e.ctrlKey && (code == 86 || code == 90 || code == 89)){
                        _delayRun(function(){
                            _delayRun(function(){g._autoCompleteQuery(queryed = input.value)},600);
                        },1)
                    }
                    if(g.autoShown && (code == 40 || code == 38 || code == 9)){
                        var index = g.selectedIndex;
                        if(code == 38){
                            g._selectItem(index == null ? -1 : index - 1,true);
                        }else if(code == 40){
                            g._selectItem(index == null ? 0 : index + 1,true);
                        }else{
                            g._hideAutoComplete();
                        }
                    }
        ...
        _setWidth: function (value)
        {
            ...
            {
                g.wrapper.css({ width: value });
                g.inputText.css({ width: value - (p.popup?20:0)});
            }
        },
        //modify by huazhen.wu@hand-china.com end
        _setHeight: function (value)
        ...
        //设置值到  隐藏域
        setValue: function (value, text ,inGrid)//modify by huazhen.wu@hand-china.com
        {
            var g = this, p = this.options;
            if (arguments.length >= 2)
            {
        ...
        _setGrid: function (value)
        {
            ...
            this.bind('buttonClick', function ()
            {
                ...
                //njq.niu@hand-china.com 重新初始化lov
                //if (!g.popupFn) {
                if(true){
                    var options = {
                        grid: gridOptions,
                        ...
                        //start 增加title,width,height njq.niu@hand-china.com
                        title:p.title,
                        width:gridOptions.width,
                        height:gridOptions.height,
                        //end 增加title,width,height njq.niu@hand-china.com
                        textField: p.textField,
                        onLoadData:p.onLoadData,
                        split: p.split,
                        ...
                        selectInit: function (rowdata)
                        {
                            ...
                            if (!value) return false;
//                            if (!value){
//                                return rowdata.__index == 0;//modify by huazhen.wu@hand-china.com
//                            }
                            if (!p.valueField || !rowdata[p.valueField]) return false;
                            return $.inArray(rowdata[p.valueField].toString(), value.split(p.split)) != -1;
                        }
    ...
    $.ligerui.getPopupFn = function (p)
    {
        ...
        function show()
        {
            ...
            var panle = $("<div></div>");
            var conditionPanel = $("<form></form>");
            var gridPanel = $("<div></div>");
            panle.append(conditionPanel).append(gridPanel);
            
            /**
             *  (1)解决bug 调整condition的位置在window后,否则form生成失败!
             *  (2)解决多语言问题
             *  
             *  njq.niu@hand-china.com
             */
            var gridParm = $.extend({
                ...
                pageSizeBtn:false,
                refreshBtn:false,
                selectByKey:true//add by huazhen.wu@hand-china.com
            }, p.grid, {
                width: "99%",
            ...
            win = $.ligerDialog.open({
                ...
                buttons: [
                 //modify by njq.niu@hand-china.com
                 { text: '选择', attr:{id:'pop_ok'}, onclick: function (item, dialog) { toSelect(); dialog.close(); } },
                 { text: '取消', attr:{id:'pop_cancel'}, onclick: function (item, dialog) { dialog.close(); } }
                ]
            });
          ...
          //搜索按钮
            if (p.condition)
            {
                ...
                $("div", containerBtn1).ligerButton({
                    ...
                    click: function ()
                    {
                        //修改了搜索逻辑 njq.niu@hand-china.com
                        var para = {},hasParam = false;
                        if(condition){
                            para = condition.getData();
                            for ( var key in para) {
                                if (!para[key]) {
                                    delete para[key]
                                }else{
                        ...
                        if(p.grid.needQueryParam && hasParam == false) {
                            //$.ligerDialog.alert('请输入查询条件!', $l('hap.tip.info'), 'alert');
                            var manager = $.ligerDialog.open({ cls: 'alert', type: 'alert', content: '请输入查询条件!', allowClose: false });
                            setTimeout(function (){
                                manager.close();
                            }, 1000);
                            return;
            ...
            grid.refreshSize();
            grid.element.focus();//add by huazhen.wu@hand-china.com
        }
...
(function ($)
{
    ...
    $.ligerui.controls.Spinner.ligerExtend($.ligerui.controls.Input, {
        ...
        {
          //modify by njq.niu@hand-china.com
            $.ligerui.controls.Spinner.base._render.call(this);
            var g = this, p = this.options;
            g.interval = null;
            g.inputText = null;
            g.value = null;
...
//            $("> .l-tab-content-item[tabid=" + tabid + "]", g.tab.content).show().siblings().hide();
            //modify by njq.niu@hand-china.com
            $("> .l-tab-content-item[tabid=" + tabid + "]", g.tab.content).removeClass('l-tab-hide').siblings().addClass('l-tab-hide')
...
//                $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).prev().show();
                $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).prev().removeClass('l-tab-hide')
...
//                $(".l-tab-content-item[tabid=" + tabid + "]", g.tab.content).prev().show();
        ...
        onResize: function ()
        {
            ...
            if (g.tab.parent()[0].tagName.toLowerCase() == "body")
            {
                ...
                g.height = p.heightDiff + windowHeight * parseFloat(p.height) * 0.01;
            }
            else
            {
...
(function ($)
{
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            //modify by njq.niu@hand-china.com
            $.ligerui.controls.TextBox.base._render.call(this);
            var g = this, p = this.options;
            g.inputText = $(this.element);
            //外层
            g.wrapper = g.inputText.wrap('<div class="l-text"></div>').parent();
        ...
        _getValue: function ()
        {
            ...
            // add by huazhen.wu@hand-china.com begin
            //大小写转换
            var v = g.inputText.val(),typecase = g.typeCase;
            if (typecase)
            {
        ...
        formatValue: function ()
        {
            ...
            } else if(p.number && p.precision && v)
            {
                ...
                g.inputText.val(p.sign ? value : value.replace('-',''));//增加负数设置 njq.niu@hand-china.com
            } 
        },
        ...
        // modify by huazhen.wu@hand-china.com begin
        setValue: function (value,inGrid)
        {
            this._setValue(value);
            if(!inGrid)
        ...
        // add by huazhen.wu@hand-china.com begin
        //大小写转换
        _setTypeCase: function (typecase)
        {
    ...
    function currencyFormatter(num, precision,isSign)
    { 
        var cents, sign;
        if (!num) num = 0;
        ...
        if (isNaN(num))
            num = 0;
        sign = !isSign ? true :(num == (num = Math.abs(num)));
...
/*
    ...
    $.ligerui.controls.TextArea.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            $.ligerui.controls.TextArea.base._render.call(this);
            var g = this, p = this.options;
            g.inputText = $(this.element);
            //外层
            g.wrapper = g.inputText.wrap('<div class="l-text"></div>').parent();
        ...
        _setDisabled: function (value)
        {
            ...
            if (value)
            {
                this.inputText[0].readOnly = true;
                this.wrapper.addClass("l-text-disabled");
            }
            else if (!p.readonly)
            {
        ...
        this.each(function ()
        {
            ...
            if (p.auto || options == undefined)
            {
                ...
                $(this).bind('mouseover.tip', function ()
                {
                    ...
                    tipmanager.tip.css({ left: p.x, top: p.y }).show();
                    //$.ligerTip(p);
                  //modify by huazhen.wu@hand-china.com end
...
//                    }
            ...
            else
            {
                ...
                p.x = p.x || 0;
                p.y = p.y || 0;
//                $.ligerTip(p);//remove by huazhen.wu@hand-china.com
            }
            $.ligerTip(p);//add by huazhen.wu@hand-china.com start
        });
...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        iconFieldName: 'icon',
        iconType:'image', //add by huazhen.wu@hand-china.com
                    ...
                    if (p.parentIcon)
                    {
                        ...
                        if (p.iconFieldName && o[p.iconFieldName]){
                            if(p.iconType == 'image'){
                                ...
                                treehtmlarr.push('<span class="' + o[p.iconFieldName] + '" />');
                            }
                        }
                        //modify by huazhen.wu@hand-china.com end
...
(function ($)
{
    ...
    l.controls.Window.ligerExtend(l.core.Win, {
        ...
        {
            var g = this, p = this.options;
            l.win.add(this);//add by huazhen.wu@hand-china.com
            this.mask();
            this.window.show();
        },
        remove: function ()
