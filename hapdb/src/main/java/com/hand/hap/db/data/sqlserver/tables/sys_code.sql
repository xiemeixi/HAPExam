...
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID('[dbo].[sys_code_b]') AND type IN ('U'))
...
ON [PRIMARY]
GO
EXEC sp_addextendedproperty 'MS_Description', N'表ID，主键，供其他表做外键', 'SCHEMA', 'dbo', 'TABLE', 'sys_code_b', 'COLUMN', 'CODE_ID'
GO
...
-- ----------------------------
ALTER TABLE [dbo].[sys_code_b] ADD
	CONSTRAINT [PK__sys_code__D033FA86DB81D6B9] PRIMARY KEY CLUSTERED ([CODE_ID]) 
	WITH (PAD_INDEX = OFF,
		IGNORE_DUP_KEY = OFF,
		ALLOW_ROW_LOCKS = ON,
		ALLOW_PAGE_LOCKS = ON)
...
-- ----------------------------
CREATE NONCLUSTERED INDEX [SYS_CODE_B_U1]
...
-- ----------------------------
--  Options for table sys_code_b
-- ----------------------------
ALTER TABLE [dbo].[sys_code_b] SET (LOCK_ESCALATION = TABLE)
GO
...
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID('[dbo].[sys_code_tl]') AND type IN ('U'))
...
ON [PRIMARY]
GO
EXEC sp_addextendedproperty 'MS_Description', N'表ID，主键，供其他表做外键', 'SCHEMA', 'dbo', 'TABLE', 'sys_code_tl', 'COLUMN', 'CODE_ID'
GO
...
-- ----------------------------
ALTER TABLE [dbo].[sys_code_tl] ADD
	CONSTRAINT [PK__sys_code__469B171164E37206] PRIMARY KEY CLUSTERED ([CODE_ID],[LANG]) 
	WITH (PAD_INDEX = OFF,
		IGNORE_DUP_KEY = OFF,
		ALLOW_ROW_LOCKS = ON,
		ALLOW_PAGE_LOCKS = ON)
...
-- ----------------------------
--  Options for table sys_code_tl
-- ----------------------------
ALTER TABLE [dbo].[sys_code_tl] SET (LOCK_ESCALATION = TABLE)
GO
